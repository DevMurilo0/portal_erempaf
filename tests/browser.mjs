...
async function login(email) {
    ...
    // Dá tempo para o Firebase concluir a transição de sessão anterior.
    await page.waitForTimeout(200);

    await page.locator('#login-email').fill(email);
    await page.locator('#login-senha').fill('TesteSeguro123!');

    assert.equal(await page.locator('#login-email').inputValue(), email);
    assert.equal(await page.locator('#login-senha').inputValue(), 'TesteSeguro123!');

    await page.locator('#btn-login').click();

    await page.waitForFunction(() =>
        document.querySelector('#tela-login').classList.contains('hidden') ||
        document.querySelector('#login-erro').textContent.trim()
    );

    assert.ok(
        await page.locator('#tela-login').evaluate(el => el.classList.contains('hidden')),
        await page.locator('#login-erro').textContent()
    );
}
...
try {
    ...
    await login(reader); await page.waitForFunction(() => document.querySelector('#campo-avisos').value === 'Aviso de teste');
    assert.ok(await page.locator('#btn-editar').isDisabled());
    report.push('Login é obrigatório; leitor autenticado acessa dados, mas localStorage não libera edição.');
    ...
    assert.ok((await page.locator('#dias-grid').textContent()).includes('<img src=x onerror="window.xss=1">'));
    assert.equal(await page.evaluate(() => window.xss), undefined);
